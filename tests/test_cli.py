...
class TestCLI:
    ...
    def test_bearer_token_env_var(
        self, clean_env, sample_endpoint, sample_bearer_token
    ):
    ...
    def test_cli_bearer_token_takes_precedence(self, clean_env, sample_endpoint):
        ...
        with patch("mcp_proxy_sigv4.cli.ProxyServer") as mock_proxy:
            ...
            runner = CliRunner()
            runner.invoke(
                main, ["--endpoint", sample_endpoint, "--bearer-token", cli_token]
            )
    ...
    def test_bearer_token_env_var_and_no_auth_conflict(
        self, clean_env, sample_endpoint, sample_bearer_token
    ):
        ...
        runner = CliRunner()
        result = runner.invoke(main, ["--endpoint", sample_endpoint, "--no-auth"])
        assert result.exit_code == 1
        assert "Cannot specify both bearer token and --no-auth" in result.output
